...
class Transform_v2(Transform):
    ...
    def match(self, u_, u, a, m_type='id'):
        index, p = self.index, self.p
        adjustment, ad_list = self.adjustment, self.ad_list

        correction1, correction2 = th.zeros_like(u_[0, :]), th.zeros_like(u_[0, :])  
        if m_type == 'id':
            task1 = dist.isend(u[ad_list[0][0] - adjustment[0], :], (index[0] + index[1] * p - 1) % p ** 2, tag=0)  # attention!
            task2 = dist.isend(u[ad_list[1][0] - adjustment[0] - 1, :], (index[0] + index[1] * p + 1) % p ** 2, tag=1)
        elif m_type == 're':
            task1 = dist.isend(u[ad_list[0][0] - adjustment[0], adjustment[1]::2].contiguous(), (index[0] + index[1] * p - 1) % p ** 2, tag=0)
            task2 = dist.isend(u[ad_list[1][0] - adjustment[0] - 1, adjustment[1]::2].contiguous(), (index[0] + index[1] * p + 1) % p ** 2, tag=1)
        ...
        if m_type == 'id':
            task1 = dist.isend(u[:, ad_list[2][1] - adjustment[1]].contiguous(), (index[0] + index[1] * p - p) % p ** 2, tag=0)
            task2 = dist.isend(u[:, ad_list[3][1] - adjustment[1] - 1].contiguous(), (index[0] + index[1] * p + p) % p ** 2, tag=1)
        elif m_type == 're':
            task1 = dist.isend(u[adjustment[0]::2, ad_list[2][1] - adjustment[1]].contiguous(), (index[0] + index[1] * p - p) % p ** 2, tag=0)
            task2 = dist.isend(u[adjustment[0]::2, ad_list[3][1] - adjustment[1] - 1].contiguous(), (index[0] + index[1] * p + p) % p ** 2, tag=1)
